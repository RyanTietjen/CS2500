...
(define (viable-clubs alos students num)
  (local [; string-in-list? : (X) X [List-of X] -> Boolean
          ; is the supplied string in the list of strings?
          (define (string-in-list? str los)
            (ormap (λ (x) (string=? str x)) los))

          ; num-of-interests : String -> Nat
          ; finds the number of times a given interest appears in the students
          ; list of given interests
          (define (num-of-interests str)
            (length
             (filter (λ (x) x)
                     (map (λ (x) (string-in-list? str (student-interests x))) students))))

          ; interest-meets-threshold?: String -> String
          ; determines if a particular interest meets the required threshold
          ; then returns the name if the interest if true,
          ; and returns the empty string if false
          (define (interest-meets-threshold? str)
            (if (>= (num-of-interests str) num)
                str
                ""))]
    (filter (λ (x) (not (string=? x ""))) (map interest-meets-threshold? alos))))
