...
(define (any-pop? pl)
  (ormap song-is-pop? (pl-songs pl)))

;; song-is-pop? : Song -> Boolean
;; determines if a song is pop or not
(check-expect (song-is-pop? SONG-1) #false)
(check-expect (song-is-pop? SONG-2) #true)

(define (song-is-pop? song)
...
(define (only-faves pl)
  (make-pl "Faves" (filter song-fav? (pl-songs pl))))
...
;; short-song? Song -> Boolean
;; returns false if a song is longer than 180 seconds (three minutes)
(check-expect (short-song? SONG-1) #true)
(check-expect (short-song? SONG-2) #false)

(define (short-song? song)
...
;; finds the total duration of the songs on a playlist
(check-expect (total-duration PL-0) 0)
(check-expect (total-duration PL-1) 840)

(define (total-duration pl)
  (foldr add-duration 0 (pl-songs pl)))

;; add-duration : Song Nat -> Nat
;; adds the duration of a song to a given number of seconds 
(check-expect (add-duration SONG-1 0) 174)
(check-expect (add-duration SONG-2 174) 415)

(define (add-duration s1 num)
  (+ (song-duration s1) num))
