...
#;(define (total-files d)
    (cond [(empty? (dir-dirs d)) 0]
          [(cons? (dir-dirs d)) (length (dir-files d))
                                (total-files (first (dir-dirs d)))
                                (total-files (rest (dir-dirs d)))]))
...
#;(define (file-found? d str)
    (if (empty? (dir-dirs d))
        (ormap (string=? (dir-files d)))
        (map (file-found? (dir-dirs d)))))

#;(define (file-found? d str)
    (local
      [(or (ormap (string=? (dir-dirs d))))
       (ormap (λ (file) (string=?) (file-name file) str)) (dir-files d)]))
...
(define (rename-files dir src dest)
  (local [; rename-files/dir : Directory -> Directory
          ; renames the files in the supplied directory
...
(define (big-files dir num)'
  (local [; file-big-enough? : File -> Boolean
          ...
          ; returns the file names to keep based on size
          (define (files-to-keep lof)
            (map file-name (filter file-big-enough? lof)))
