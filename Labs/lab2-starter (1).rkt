...
(define (string-starts-with? str1 str2)
  (cond [(and (= (string-length str1) 0) (= (string-length str2) 0)) #t]
        [(string=? str1 str2) #t]
        [(and (> (string-length str1) (string-length str2))
                  (string=? (substring str1 0 (string-length str2)) str2)) #t]
        [else #f]))
...
; Template
(define (door-state-temp str)
  (cond [(string=? str DS-CLOSED) ...]
        [(string=? str DS-LOCKED) ...]
        [(string=? str DS-OPEN) ...]))
...
(define (draw-door ds)
  (cond [(string=? ds DS-CLOSED) DOOR-CLOSED]
...
(define (change-locked-door ke)
  (if (key=? ke "u") DS-CLOSED DS-LOCKED))
