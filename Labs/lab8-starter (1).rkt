...
(check-expect (palindrome? "hello") #f)
(check-expect (palindrome? "Racecar") #t)

(define (palindrome? str)
  (string-ci=?
   str
   (foldl string-append "" (explode str))))
...
;; ls->color-fn : LetterStatus -> Image
;; produces a sized background for a particular guess using lambda
(check-expect ((ls->color-fn LS-WRONG) 50) (square 50 "solid" "dimgray"))
(check-expect ((ls->color-fn LS-MISPLACED) 50) (square 50 "solid" "goldenrod"))
(check-expect ((ls->color-fn LS-RIGHT) 50) (square 50 "solid" "darkgreen"))
...
;; stack : [List-of Image] [Image Image -> Image]  -> Image
;; stacks images either horizontally or vertically

(define (stack images placement)
  (local [; buffer : Image Image -> Image
          ; positions the supplied image relative to the supplied background with a gap
          (define (buffer i bg)
            (placement GAP i bg))]
