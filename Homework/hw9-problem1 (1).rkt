...
(define (read-dictionary fpath)
  (if (file-exists? fpath)
      (map string-upcase (read-lines fpath))
      '()))
...
; ls : String String -> [List-of LetterStatusPair]
...
(define (score str1 str2)
  (local [(define correct-letters
            (mymap2 (explode str1) (explode str2) string=?))
          (define correct-or-misplaced-letters
            (map (λ (str) (string-in-list? str (explode str2))) (explode str1)))
          ; convert-guesses-to-status : Boolean Boolean -> LetterStatus
          ; produces the corresponding LetterStatus if a guess is right or misplaced
          (define (convert-guesses-to-status correct misplaced)
            (cond [correct LS-RIGHT]
                  [misplaced LS-MISPLACED]
                  [else LS-WRONG]))]
    (mymap2 (explode str1)
            (mymap2 correct-letters correct-or-misplaced-letters convert-guesses-to-status)
            make-pair)))
...
(define (partial-guess->full-guess str num)
  (implode
   (append (explode str)
           (build-list (- num (string-length str)) (λ (n) " ")))))
...
(define WS-WORD4 (make-wordle (list "CAT" "DAB" "BAD") "HE"))
...
(define (key-wordle ws ke alos num)
  (cond [(string=? ke "\b") (key-wordle-backspace ws)]
        [(string=? ke "\r") (key-wordle-return ws alos num)]
        [else (key-wordle-letter ws ke num)]))


;; key-wordle-backspace : WS -> WS
;; hands the key wordle function if the input is a backspace
(check-expect (key-wordle-backspace WS-START) WS-START)
(check-expect (key-wordle-backspace WS-TYPE1) WS-START)
(check-expect (key-wordle-backspace WS-TYPE2) WS-TYPE1)
(check-expect (key-wordle-backspace WS-TYPE3) WS-TYPE2)
(check-expect (key-wordle-backspace WS-TYPE-BAD) WS-TYPE2)
(check-expect (key-wordle-backspace WS-WORD1) WS-WORD1)

(define (key-wordle-backspace ws)
  (make-wordle (wordle-past ws)
               (if (string=? (wordle-current ws) "")
                   ""
                   (substring (wordle-current ws) 0 (- (string-length (wordle-current ws)) 1)))))

;; key-wordle-return : WS [List-of String] Nat -> WS
;; hands the key wordle function if the input is return
(check-expect (key-wordle-return WS-START LITTLE-WORDS 3) WS-START)
(check-expect (key-wordle-return WS-TYPE1 LITTLE-WORDS 3) WS-TYPE1)
(check-expect (key-wordle-return WS-TYPE2 LITTLE-WORDS 3) WS-TYPE2)
(check-expect (key-wordle-return WS-TYPE3 LITTLE-WORDS 3) WS-WORD1)
(check-expect (key-wordle-return WS-TYPE-BAD LITTLE-WORDS 3) WS-TYPE-BAD)
(check-expect (key-wordle-return WS-WORD1 LITTLE-WORDS 3) WS-WORD1)

(define (key-wordle-return ws alos num)
  (if (and (= (string-length (wordle-current ws)) num)
           (string-in-list? (wordle-current ws) alos))
      (make-wordle (append (wordle-past ws) (list (wordle-current ws))) "")
      ws))

;; key-wordle-letter : WS KeyEvent Nat -> WS
;; hands the key wordle function if the input is return
(check-expect (key-wordle-letter WS-START "C" 3) WS-TYPE1)
(check-expect (key-wordle-letter WS-START "c" 3) WS-TYPE1)
(check-expect (key-wordle-letter WS-START "1" 3) WS-START)
(check-expect (key-wordle-letter WS-TYPE1 "A" 3) WS-TYPE2)
(check-expect (key-wordle-letter WS-TYPE1 "A" 1) WS-TYPE1)
(check-expect (key-wordle-letter WS-TYPE1 "2" 3) WS-TYPE1)
(check-expect (key-wordle-letter WS-TYPE2 "T" 3) WS-TYPE3)
(check-expect (key-wordle-letter WS-TYPE2 "X" 3) WS-TYPE-BAD)
(check-expect (key-wordle-letter WS-TYPE3 "S" 3) WS-TYPE3)

(define (key-wordle-letter ws ke num)
  (if (and (< (string-length (wordle-current ws)) num) (string-alphabetic? ke))
...
(define (end-wordle? ws str num)
  (or (string-in-list? str (wordle-past ws))
      (>= (length (wordle-past ws)) num)))
...
(check-expect (draw-end-wordle WS-WORD1 "CAT" 3)
              (overlay/align "middle" "middle"
                             (text "Congrats! You won in 1 guess(es)!" 24 "black")
                             (empty-scene 500 500)))
...
(check-expect (draw-end-wordle WS-WORD2 "DAB" 2)
              (overlay/align "middle" "middle"
                             (text "Congrats! You won in 2 guess(es)!" 24 "black")
                             (empty-scene 500 500)))

(check-expect (draw-end-wordle WS-WORD3 "BAD" 5)
              (overlay/align "middle" "middle"
                             (text "Congrats! You won in 3 guess(es)!" 24 "black")
                             (empty-scene 500 500)))



(define (draw-end-wordle ws str num)
  (if (string-in-list? str (wordle-past ws))
      (overlay/align "middle" "middle"
                     (text (string-append "Congrats! You won in "
...
(check-expect (draw-wordle WS-START "BUG" 5)
              (overlay/align "middle" "middle"
                             (stack/v (list (partial-guess->image "" 3)
                                            (partial-guess->image "" 3)
                                            (partial-guess->image "" 3)
                                            (partial-guess->image "" 3)
                                            (partial-guess->image "" 3)))
...
(check-expect (draw-wordle WS-WORD4 "ETA" 5)
              (overlay/align "middle" "middle"
                             (stack/v (list (stack/h (list
                                                      (lsp->image (make-pair "C" LS-WRONG))
                                                      (lsp->image (make-pair "A" LS-MISPLACED))
                                                      (lsp->image (make-pair "T" LS-MISPLACED))))
                                            (stack/h (list
                                                      (lsp->image (make-pair "D" LS-WRONG))
                                                      (lsp->image (make-pair "A" LS-MISPLACED))
                                                      (lsp->image (make-pair "B" LS-WRONG))))
                                            (stack/h (list
                                                      (lsp->image (make-pair "B" LS-WRONG))
                                                      (lsp->image (make-pair "A" LS-MISPLACED))
                                                      (lsp->image (make-pair "D" LS-WRONG))))
                                            (partial-guess->image "HE" 3)
                                            (partial-guess->image "" 3)))
                             (empty-scene 500 500)))

(check-expect (draw-wordle WS-TYPE2 "RUN" 5)
              (overlay/align "middle" "middle"
                             (stack/v (list (partial-guess->image "CA" 3)
                                            (partial-guess->image "" 3)
                                            (partial-guess->image "" 3)
                                            (partial-guess->image "" 3)
                                            (partial-guess->image "" 3)))
...
(define (draw-wordle ws str num)
  (local [; word->visualization : String String -> Image
          ; creates a visualization for a given word in a WordleState
          (define (word->visualization guess correct-word)
            (stack/h (map lsp->image (score guess correct-word))))
          (define NUM-OF-WORDS-GUESSED (length (wordle-past ws)))]
    (overlay/align "middle" "middle"
                   (stack/v
                    (append
                     (map (λ (ws-guess) (word->visualization ws-guess str)) (wordle-past ws))
                     (list (partial-guess->image (wordle-current ws) (string-length str)))
                     (build-list (- (- num NUM-OF-WORDS-GUESSED) 1)
                                 (λ (n) (partial-guess->image "" (string-length str))))))
                   (empty-scene 500 500))))
