...
(define (count-strings alos)
  (local [; add-count : String [List-of SC] -> [List-of SC]
          ; either creates a new string in the list of StringCounts or
          ; increments the StringCount of the matching string in the list
          (define (add-count str acc)
            (cond
              [(empty? acc) (list (make-sc str 1))]
              [(string=? str (sc-str (first acc)))
               (cons (make-sc str (add1 (sc-count (first acc)))) (rest acc))]
              [else (cons (first acc) (add-count str (rest acc)))]))


          ; count-strings/a [List-of String] [List-of SC] -> [List-of SC]
          ; counts the distinct strings in a list
          ; Accumulator: current list of SC
          (define (count-strings/a alos acc)
            (cond
              [(empty? alos) acc]
              [(cons? alos)
               (count-strings/a (rest alos) (add-count (first alos) acc))]))]
    
    (count-strings/a alos '())))
