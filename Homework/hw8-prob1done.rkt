...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname hw8-prob1done) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; produces a list of shelves containing only authors that don't
; belong on the shelf from a given list of shelves

(define (fix-shelves shelves)
  (filter (λ (shelf) (not (empty? (shelf-authors shelf)))) (map fix shelves)))

(check-expect (fix SHELF-1) (make-shelf "A" (list "Hurston" "Butler")))
(check-expect (fix SHELF-2) (make-shelf "B" '()))
(check-expect (fix SHELF-3) (make-shelf "C" '()))

; fix : Shelf -> Shelf
; removes all authors who belong in a given shelf,
; leaving only the authors who do not belong
                                                                        
(define (fix shelf)
  (make-shelf (shelf-letter shelf)
              (filter (λ (author)
                        (not (string=? (shelf-letter shelf) (substring author 0 1))))
                      (shelf-authors shelf))))
