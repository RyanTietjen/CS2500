...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname hw8-prob3done) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(check-expect (viable-fish (list "Snakehead" "Guppy") (list FISH-1 FISH-3 FISH-4)) (list FISH-1))
(check-expect (viable-fish (list "Carp" "Lake Trout") (list FISH-1 FISH-2 FISH-4)) (list FISH-4))
(check-expect (viable-fish '() (list FISH-1 FISH-3 FISH-4)) (list FISH-1 FISH-4))
(check-expect (viable-fish (list "Guppy") '()) '())

; viable-fish : [List-of String] [List-of Fish] -> [List-of Fish]
; removes any invasive or fish nine inches
; or less from a given list of caught fish

(define (viable-fish los lof)
  (local [; meanie? : Fish -> Boolean
          ; returns #false if the fish is invasive
          (define (meanie? fish)
            (andmap (λ (invasive)
                      (not (string=? (fish-species fish) invasive))) los))]
    (filter (λ (fish) (> (fish-length-in-inches fish) 8))
            (filter meanie? lof))))
