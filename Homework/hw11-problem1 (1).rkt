...
(define (in-graph? num g)
  (ormap (λ (n) (= num n)) (graph-v g)))
...
(define (change-color c1 c2 g)
  (make-graph (graph-v g)
              (λ (verticy) (if (string=? c1 ((graph-c g) verticy))
                               c2
                               ((graph-c g) verticy)))
              (graph-e g)))
...
; node-neighbors ; Nat Graph -> [List-of Nat]
; finds the vertices of the nodes connected to that node
(check-expect (node-neighbors 1 GRAPH-123) (list 2))
(check-expect (node-neighbors 2 GRAPH-123) '())
(check-expect (node-neighbors 3 GRAPH-123) '())

(check-expect (node-neighbors 7 GRAPH-789) (list 8))
(check-expect (node-neighbors 8 GRAPH-789) (list 7 9))
(check-expect (node-neighbors 9 GRAPH-789) '())

(define (node-neighbors num g)
  (filter (λ (x) ((graph-e g) num x)) (graph-v g)))
...
(define (has-a-dot? g)
  (local [; list-connected-nodes : [List-of Nat] -> [List-of Nat]
          ; produces a list of nodes that are either connected to a node
          ; or connect to a node, then removes duplicates and sorts it
          (define (list-connected-nodes alon)
            (sort (foldr append '() (map (λ (node) (node-neighbors node g)) (graph-v g))) <))

          ; equal-vertices? : [List-of Nat] [List-of Nat] -> Boolean
          ; determines if two lists of numbers are the sme
          (define (equal-vertices? alon1 alon2)
            (cond [(and (empty? alon1) (empty? alon2)) #t]
                  [(or (empty? alon1) (empty? alon2)) #f]
                  [(and (cons? alon1) (cons? alon2))
                   (if (= (first alon1) (first alon2))
                       (equal-vertices? (rest alon1) (rest alon2))
                       #f)]))]
    (not (equal-vertices? (list-connected-nodes (graph-v g)) (graph-v g)))))
...
(check-expect (can-reach-in-time? 1 3 1 GRAPH-123) #false)
...
(define (can-reach-in-time? start end steps g)   
  (cond [(zero? steps) (= start end)]
        [(positive? steps)
         (or (= start end)
             (ormap (λ (nid) (can-reach-in-time? nid end (sub1 steps) g))
                    (node-neighbors start g)))]))
